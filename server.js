...
function processTracking(trackingList, ordersByCourierList) {
  ...
  for (const [id, cs] of courierMap) {
    ...
    // Coords: última válida; se faltar, cai pra referência do "parado" (nunca sumir sem posição)
    const clat2 = (typeof cs.lat === 'number') ? cs.lat : (typeof cs.stLat === 'number' ? cs.stLat : null);
    const clng2 = (typeof cs.lng === 'number') ? cs.lng : (typeof cs.stLng === 'number' ? cs.stLng : null);
